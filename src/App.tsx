...
const App: React.FC = () => {
  ...
  const handleSearch = useCallback(async (keywords: string, location: string, profileSummary: string) => {
    setIsLoading(true);
    setError(null);
    setJobs([]);
    ...
    try {
      console.log("Calling n8n webhook with:", { keywords, location, profileSummary });
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData) {
             ...
             errorMessage = errorData.errorDescription || errorData.errorMessage || errorData.message || errorMessage;
          }
        } catch (e) {
      ...
      if (err.name === 'AbortError') {
        ...
        const specificError = err.message && err.message !== 'Failed to fetch' ? err.message : genericError;
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-3xl mx-auto">
          <p className="text-center text-slate-400 mb-8 -mt-2">{t('header.tagline')}</p>
          ...
          <div className="mt-12">
            ...
            {!isLoading && !error && jobs.length > 0 && (
              <div className="space-y-6">
                 <h2 className="text-2xl font-bold text-center text-slate-200">{t('app.topMatches')}</h2>
                {jobs.map((job, index) => (
                  <JobCard key={job.url || index} job={job} onGenerateCoverLetter={handleGenerateCoverLetter} />
                ))}
              </div>
            )}
