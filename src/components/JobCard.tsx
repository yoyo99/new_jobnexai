...
export const JobCard: React.FC<JobCardProps> = ({ job, onGenerateCoverLetter }) => {
  ...
  return (
    <div className="bg-slate-800 rounded-lg p-6 shadow-lg hover:shadow-xl transition-shadow border border-slate-700">
      <div className="flex items-start justify-between mb-4">
        ...
        {job.company_logo && (
          <img src={job.company_logo} alt={job.company} className="w-16 h-16 rounded object-cover ml-4" />
        )}
      </div>
      ...
      <div className="flex gap-3">
        <a 
          ...
          className="flex-1 bg-blue-600 hover:bg-blue-700 text-white text-center py-2 px-4 rounded-md transition-colors font-medium"
        >
        ...
        >
          {t('job.generateLetter') || 'Generate Cover Letter'}
        </button>
      </div>
