  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'update_preferences') {
        ...
        return new Response(
          JSON.stringify({ success: true, message: 'Préférences mises à jour' }),
          { 
            status: 200, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          }
      ...
      if (action === 'send_notifications') {
        // Envoyer les notifications pour les nouvelles offres
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'preferences' && userId) {
        ...
        return new Response(
          JSON.stringify({ preferences: preferences || getDefaultPreferences() }),
          { 
            status: 200, 
            headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
          }
...
async function processJobNotifications(): Promise<any[]> {
  ...
  try {
    // Récupérer les nouvelles offres d'emploi (dernières 24h)
    ...
    // Pour chaque utilisateur, vérifier s'il y a des offres correspondantes
    for (const user of users || []) {
      ...
      if (matchingJobs.length > 0) {
        ...
        const notification = {
          ...
          message: `Nous avons trouvé ${matchingJobs.length} nouvelle(s) offre(s) correspondant à vos critères.`,
          type: 'job_alert',
          data: { jobs: matchingJobs.slice(0, 5) }, // Limiter à 5 offres
          read: false,
        ...
        if (!notifError) {
          ...
          if (user.email_notifications) {
            await sendEmailNotification(user.profiles.email, user.profiles.full_name, matchingJobs)
          }
        }
      }
    }
...
async function sendEmailNotification(email: string, fullName: string, jobs: JobOffer[]): Promise<void> {
  ...
  const emailContent = `
    ...
    ${jobs.map(job => `- ${job.title} chez ${job.company} (${job.location || 'Localisation non spécifiée'})`).join('\n')}
