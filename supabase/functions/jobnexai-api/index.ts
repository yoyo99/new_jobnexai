...
serve(async (req) => {
  ...
  try {
    ...
    // POST /automation/log - Log action from n8n or other services
    if (resource === "automation" && id === "log" && req.method === "POST") {
      const body = await req.json();
    ...
    // GET /automation/logs/{userId}
    if (resource === "automation" && id === "logs" && action) {
